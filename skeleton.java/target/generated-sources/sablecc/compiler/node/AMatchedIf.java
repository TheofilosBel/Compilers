/* This file was generated by SableCC (http://www.sablecc.org/). */

package compiler.node;

import compiler.analysis.*;

@SuppressWarnings("nls")
public final class AMatchedIf extends PMatchedIf
{
    private TKwIf _kwIf_;
    private PCond _cond_;
    private TKwThen _kwThen_;
    private PMatched _inner_;
    private TKwElse _kwElse_;
    private PMatched _outer_;

    public AMatchedIf()
    {
        // Constructor
    }

    public AMatchedIf(
        @SuppressWarnings("hiding") TKwIf _kwIf_,
        @SuppressWarnings("hiding") PCond _cond_,
        @SuppressWarnings("hiding") TKwThen _kwThen_,
        @SuppressWarnings("hiding") PMatched _inner_,
        @SuppressWarnings("hiding") TKwElse _kwElse_,
        @SuppressWarnings("hiding") PMatched _outer_)
    {
        // Constructor
        setKwIf(_kwIf_);

        setCond(_cond_);

        setKwThen(_kwThen_);

        setInner(_inner_);

        setKwElse(_kwElse_);

        setOuter(_outer_);

    }

    @Override
    public Object clone()
    {
        return new AMatchedIf(
            cloneNode(this._kwIf_),
            cloneNode(this._cond_),
            cloneNode(this._kwThen_),
            cloneNode(this._inner_),
            cloneNode(this._kwElse_),
            cloneNode(this._outer_));
    }

    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAMatchedIf(this);
    }

    public TKwIf getKwIf()
    {
        return this._kwIf_;
    }

    public void setKwIf(TKwIf node)
    {
        if(this._kwIf_ != null)
        {
            this._kwIf_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._kwIf_ = node;
    }

    public PCond getCond()
    {
        return this._cond_;
    }

    public void setCond(PCond node)
    {
        if(this._cond_ != null)
        {
            this._cond_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._cond_ = node;
    }

    public TKwThen getKwThen()
    {
        return this._kwThen_;
    }

    public void setKwThen(TKwThen node)
    {
        if(this._kwThen_ != null)
        {
            this._kwThen_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._kwThen_ = node;
    }

    public PMatched getInner()
    {
        return this._inner_;
    }

    public void setInner(PMatched node)
    {
        if(this._inner_ != null)
        {
            this._inner_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._inner_ = node;
    }

    public TKwElse getKwElse()
    {
        return this._kwElse_;
    }

    public void setKwElse(TKwElse node)
    {
        if(this._kwElse_ != null)
        {
            this._kwElse_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._kwElse_ = node;
    }

    public PMatched getOuter()
    {
        return this._outer_;
    }

    public void setOuter(PMatched node)
    {
        if(this._outer_ != null)
        {
            this._outer_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._outer_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._kwIf_)
            + toString(this._cond_)
            + toString(this._kwThen_)
            + toString(this._inner_)
            + toString(this._kwElse_)
            + toString(this._outer_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._kwIf_ == child)
        {
            this._kwIf_ = null;
            return;
        }

        if(this._cond_ == child)
        {
            this._cond_ = null;
            return;
        }

        if(this._kwThen_ == child)
        {
            this._kwThen_ = null;
            return;
        }

        if(this._inner_ == child)
        {
            this._inner_ = null;
            return;
        }

        if(this._kwElse_ == child)
        {
            this._kwElse_ = null;
            return;
        }

        if(this._outer_ == child)
        {
            this._outer_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._kwIf_ == oldChild)
        {
            setKwIf((TKwIf) newChild);
            return;
        }

        if(this._cond_ == oldChild)
        {
            setCond((PCond) newChild);
            return;
        }

        if(this._kwThen_ == oldChild)
        {
            setKwThen((TKwThen) newChild);
            return;
        }

        if(this._inner_ == oldChild)
        {
            setInner((PMatched) newChild);
            return;
        }

        if(this._kwElse_ == oldChild)
        {
            setKwElse((TKwElse) newChild);
            return;
        }

        if(this._outer_ == oldChild)
        {
            setOuter((PMatched) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
